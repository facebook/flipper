...
class NetworkViewController: UIViewController {
  ...
  @IBAction func tappedGetAPI(_ sender: UIButton) {
    ...
    let dataTask = URLSession.shared.dataTask(with: getURL) { [weak self] (data, response, error) in
      ...
      let dict = try? JSONSerialization.jsonObject(with: dataUnwrapped, options: JSONSerialization.ReadingOptions(rawValue: 0)) as? [String: String]
  ...
  @IBAction func tappedPOSTAPI(_ sender: UIButton) {
    ...
    postRequest.httpBody = try! JSONSerialization.data(withJSONObject: dict, options: JSONSerialization.WritingOptions(rawValue: 0))
  ...
  func showAlert(message: String) {
    DispatchQueue.main.async {
      let alertController = UIAlertController(title: "Flipper", message: message, preferredStyle: .alert)
