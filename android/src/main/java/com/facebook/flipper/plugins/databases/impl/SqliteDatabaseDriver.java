...
public class SqliteDatabaseDriver extends DatabaseDriver<SqliteDatabaseDescriptor> {
    ...
    public SqliteDatabaseDriver(final Context context) {
        this(context, new SqliteDatabaseProvider() {
            @Override
    ...
    public SqliteDatabaseDriver(final Context context, final SqliteDatabaseProvider sqliteDatabaseProvider) {
        this(context, sqliteDatabaseProvider, new SqliteDatabaseConnectionProvider() {
            @Override
    ...
    public SqliteDatabaseDriver(final Context context, final SqliteDatabaseProvider sqliteDatabaseProvider, final SqliteDatabaseConnectionProvider sqliteDatabaseConnectionProvider) {
        super(context);
    ...
    public DatabaseGetTableStructureResponse getTableStructure(@NonNull SqliteDatabaseDescriptor databaseDescriptor, @NonNull String table) {
        ...
        try {
            ...
            Cursor definitionCursor = database.rawQuery("SELECT sql FROM " + SCHEMA_TABLE + " WHERE name=?", new String[]{table});
            try {
                ...
                while (foreignKeysCursor.moveToNext()) {
                    foreignKeyValues.put(
                        ...
                        foreignKeysCursor.getString(foreignKeysCursor.getColumnIndex("table")) + "(" + foreignKeysCursor.getString(foreignKeysCursor.getColumnIndex("to")) + ")"
                    );
                }
                ...
                while (structureCursor.moveToNext()) {
                    ...
                    structureValues.add(
                        Arrays.asList(
                            ...
                            getObjectFromColumnIndex(structureCursor, structureCursor.getColumnIndex("dflt_value")),
                ...
                return new DatabaseGetTableStructureResponse(structureColumns, structureValues, indexesColumns, indexesValues, definition);
            } finally {
    ...
    private static List<List<Object>> cursorToList(Cursor cursor) {
        ...
        while (cursor.moveToNext()) {
            ...
            for (int column = 0; column < numColumns; column++) {
                values.add(
                    getObjectFromColumnIndex(cursor, column)
                );
            }
            rows.add(values);
        }
